# Pitch Deck - LocalRank Feedback

> Documento de presentacion para inversores, socios y partners estrategicos.
> Cada seccion corresponde a una slide.

---

## Slide 1: Portada

**LocalRank Feedback**

*Convierte cada visita en resenas de 5 estrellas*

Plataforma SaaS de reputacion online para negocios locales.

Contacto: [tu email]
Web: localrankfeedback.com

---

## Slide 2: El Problema

**Los negocios locales pierden clientes por su reputacion online**

- El 93% de consumidores leen resenas antes de visitar un negocio local
- Un negocio con menos de 4 estrellas pierde hasta el 70% de potenciales clientes
- 1 resena negativa sin respuesta equivale a perder 30 clientes potenciales
- Los negocios no tienen forma de interceptar insatisfaccion antes de que sea publica

**Resultado:** Clinicas, fisios, dentistas y gimnasios pierden miles de dolares al mes por resenas que pudieron evitarse con una llamada a tiempo.

---

## Slide 3: La Solucion

**Un flujo post-visita que protege y potencia tu reputacion**

1. El cliente sale y califica su experiencia (link, QR, SMS)
2. Si esta satisfecho (4-5 ⭐) → lo dirigimos a Google para dejar resena publica
3. Si esta insatisfecho (1-3 ⭐) → capturamos privado + alerta inmediata al dueno por WhatsApp

**Resultado:** Mas resenas positivas, menos negativas, y la capacidad de recuperar clientes insatisfechos ANTES de que publiquen.

---

## Slide 4: Producto

**Modulos de la plataforma:**

| Modulo | Que hace |
|--------|----------|
| Feedback Post-Visita | Captura satisfaccion con formulario personalizable |
| Review Gate | Filtra: positivos a Google, negativos en privado |
| Alertas WhatsApp/Email | Notifica al dueno inmediatamente en feedback negativo |
| Respuestas IA | Responde automaticamente resenas de Google con el tono de la marca |
| Referidos | Convierte clientes satisfechos en embajadores |
| Remarketing | Captura datos con consentimiento para WhatsApp y Google Ads |
| Panel Multi-Sede | Dashboard con NPS, trends, tickets y metricas |

---

## Slide 5: Demo / Como Funciona

**Flujo en 30 segundos:**

```
Cliente sale de consulta
       ↓
Escanea QR o recibe link
       ↓
Califica 1-5 estrellas
       ↓
    ┌────────────────────────┐
    │                        │
  4-5 ⭐                  1-3 ⭐
    │                        │
    ↓                        ↓
Google Reviews         WhatsApp al dueno
(resena publica)      (alerta privada)
    │                        │
    ↓                        ↓
Rating sube            Dueno contacta
                       y recupera cliente
```

---

## Slide 6: Mercado (TAM/SAM/SOM)

**Mercado objetivo: Negocios locales con atencion presencial**

| Nivel | Mercado | Tamano |
|-------|---------|--------|
| TAM | Negocios locales globalmente | $12B (software de reputacion) |
| SAM | LATAM + Espana (negocios con GMB activo) | $1.2B |
| SOM | Colombia + expansion inicial | $80M |

**Segmentos prioritarios:**
- Clinicas dentales: 15,000+ en Colombia
- Fisioterapeutas: 8,000+
- Centros de estetica: 20,000+
- Gimnasios: 5,000+

**Solo en Colombia:** +50,000 negocios potenciales a $49-99/mes = $30M-60M ARR potencial

---

## Slide 7: Modelo de Negocio

**SaaS por suscripcion mensual por sede**

| Plan | Precio | Target |
|------|--------|--------|
| Starter | $49/mes/sede | Negocio individual, 1 sede |
| Growth | $99/mes/sede | Negocio multi-sede, quiere crecer |
| Agencia | $249/mes (10 sedes) | Agencias de marketing digital |

**Unit Economics (proyeccion):**
- ARPU: $85/mes
- CAC estimado: $150 (via Google Ads + referidos)
- LTV estimado: $1,700 (20 meses promedio)
- LTV/CAC: 11.3x
- Payback: <2 meses

**Revenue adicional:**
- Creditos SMS/WhatsApp por encima del plan
- Whitelabel premium
- Integraciones enterprise

---

## Slide 8: Traccion / Metricas

**Estado actual: Pre-lanzamiento (MVP construido)**

- Plataforma funcional (backend + frontend + panel)
- 3 negocios en piloto beta
- Formulario de feedback operativo con review gate
- Notificaciones WhatsApp + Email funcionando

**Objetivos 6 meses:**
- 50 negocios pagando
- $4,250 MRR
- 5,000 feedback procesados/mes
- Presencia en 3 ciudades

**Objetivos 12 meses:**
- 200 negocios pagando
- $17,000 MRR
- 5 agencias como partners
- Expansion a 2 paises

---

## Slide 9: Competencia

**Landscape competitivo:**

| Competidor | Que hace | Debilidad |
|-----------|---------|-----------|
| Spokk | Referidos + reviews | No tiene review gate ni alertas WhatsApp. Caro. |
| Trustmary | Reviews + NPS | Enfocado en B2B/SaaS, no negocios locales. |
| Birdeye | Reputacion enterprise | Precio minimo $300/mes. Complejo. No LATAM. |
| Podium | Messaging + reviews | Enterprise, +$400/mes. No disponible en LATAM. |
| Grade.us | White-label reviews | Solo resenas, no feedback ni referidos. |

**Nuestro diferencial:**
1. **Review Gate + Alertas WhatsApp** — Nadie combina filtro inteligente + notificacion instantanea
2. **Precio accesible para LATAM** — 5-10x mas barato que alternativas USA
3. **Todo-en-uno** — Feedback + Reviews + Referidos + Remarketing en una sola herramienta
4. **WhatsApp nativo** — Canal dominante en LATAM, no disponible en competidores USA

---

## Slide 10: Go-to-Market

**Estrategia de adquisicion:**

**Canal 1: Venta directa (Primeros 50 clientes)**
- Contacto directo a clinicas/fisios via WhatsApp e Instagram
- Oferta: 30 dias gratis + setup personalizado
- Meta: 10 clientes/mes

**Canal 2: Agencias de marketing digital**
- Partner program: agencias revenden con markup
- Whitelabel disponible desde Plan Agencia
- Meta: 5 agencias gestionando 10+ clientes cada una

**Canal 3: Contenido + SEO**
- Blog: "Como mejorar tu rating en Google Maps"
- YouTube: Tutoriales para negocios locales
- Google Ads: Keywords de intencion ("mejorar resenas Google negocio")

**Canal 4: Referidos propios**
- Negocios que usan LocalRank refieren a otros negocios
- Comision: 1 mes gratis por referido convertido

---

## Slide 11: Equipo

**Fundador(es):**

- [Tu nombre] — CEO / Producto
  - Background en [tu experiencia]
  - Conocimiento profundo del mercado local

**Necesidades de equipo (proximos 6 meses):**
- 1 Developer fullstack (ya parcialmente cubierto con IA)
- 1 Growth/Sales (contacto directo con negocios)
- 1 Customer Success (onboarding y retencion)

---

## Slide 12: Roadmap

**Q1 2025: MVP + Primeros clientes**
- Feedback + Review Gate + Alertas ✅
- Panel basico ✅
- 10-20 clientes piloto

**Q2 2025: Growth features**
- Respuestas IA a Google Reviews
- Programa de referidos
- WhatsApp masivo
- 50+ clientes

**Q3 2025: Escala**
- Plan Agencia + Whitelabel
- API de integraciones
- Expansion a Mexico
- 150+ clientes

**Q4 2025: Consolidacion**
- Analytics avanzado con IA
- Integracion con CRMs
- 300+ clientes
- Break-even

---

## Slide 13: Financials (Ask)

**Buscamos: $150,000 USD (Pre-seed / Angel)**

**Uso de fondos:**
| Rubro | % | Monto |
|-------|---|-------|
| Producto (desarrollo + infra) | 40% | $60,000 |
| Ventas y marketing | 35% | $52,500 |
| Operaciones | 15% | $22,500 |
| Legal + compliance | 10% | $15,000 |

**Con este capital:**
- 12-18 meses de runway
- 200+ clientes pagando
- $17,000+ MRR
- Posicion para levantar Seed de $500K-$1M

**Alternativa bootstrap:**
- Crecimiento organico con cash flow positivo desde mes 3
- Reinversion de ingresos
- Mas lento pero sin dilusion

---

## Slide 14: Cierre

**LocalRank Feedback**

*Protege tu reputacion. Multiplica tus resenas. Recupera clientes insatisfechos.*

- MVP funcional y listo para escalar
- Mercado de $80M+ en Colombia
- Unit economics saludables (LTV/CAC 11x)
- Diferencial claro vs competencia

**Siguiente paso:** Demo personalizada o piloto gratuito

Contacto: [tu email]
WhatsApp: [tu numero]
Web: localrankfeedback.com

---

*Pitch Deck - LocalRank Feedback - Agosto 2026*
